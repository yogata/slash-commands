...
- `git branch --merged main` — マージ済みローカルブランチ
- `git branch -r --merged main` — マージ済みリモートブランチ
...
- `git worktree remove <worktree-path>` — 削除対象worktreeの削除
- `git worktree remove --force <worktree-path>` — 強制削除が必要な場合

### 5. mainブランチに切り替え

`git checkout main` でmainブランチに切り替え、`git pull origin main` で最新化する。

### 6. ローカルブランチ削除

- `git branch -d <branch-name>` — 通常削除
- `git branch -D <branch-name>` — 削除に失敗する場合（未マージ警告）の強制削除

### 7. リモートブランチ削除

`git push origin --delete <branch-name>` でリモートブランチを削除する。

### 8. Prune実行

`git fetch --prune` でリモート追跡ブランチを整理する。
